  ...
  while( 1 )
  {
    ...
    for( unsigned int i=0; i < vertexIndices.size(); i++ ){
      unsigned int vertexIndex = vertexIndices[i];
       glm::vec3 t_vertex = temp_vertices[ vertexIndex-1 ];
       out_vertices.push_back(t_vertex);
    }
  ...
  // For each UV of each triangle
    for( unsigned int i=0; i < uvIndices.size(); i++ ){
      unsigned int uvIndex = uvIndices[i];
       glm::vec2 t_uvs = temp_uvs[ uvIndex-1 ];
       out_uvs.push_back(t_uvs);
    }

  // For each normal of each triangle
    for( unsigned int i=0; i < normalIndices.size(); i++ ){
      unsigned int normalIndex = normalIndices[i];
       glm::vec3 t_normals = temp_normals[ normalIndex-1 ];
       out_normals.push_back(t_normals);
    }
